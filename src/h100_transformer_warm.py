...
def run_transformer_warm():
    ...
    for model_config in MODEL_QUEUE:
        ...
        print("\n" + "=" * 70)
        print(f"🔥 {model_name.upper()} WARM MODE (CoT) | H100 BENCHMARK")
        print(f"Platform: {PLATFORM_NAME}")
        print(f"Target:   {len(files)} Files")
        print("=" * 70 + "\n")

        # 1. LOAD MODEL (WARM START)
        ...
        try:
            llm = Llama(
                ...
                main_gpu=0,
                n_ctx=8192,  # 8k Context for CoT Reasoning safety
                n_batch=512,
                verbose=False
            )
        ...
        for i, path in enumerate(files):
            ...
            try:
                ...
                # Generate (No json_object constraint for CoT)
                resp = llm.create_chat_completion(
                    ...
                    temperature=0.1,
                    max_tokens=1024  # Allow space for reasoning text
                )
                ...
                # Soft Reset KV Cache (Critical for keeping WARM runs clean)
                llm.reset()
        ...
        output_csv = os.path.join(LOG_DIR, f"h100_{model_name.lower()}_cot_warm.csv")
        pd.DataFrame(results).to_csv(output_csv, index=False)
        print(f"\n✅ Saved {model_name} results to: {output_csv}")

        # 4. CLEANUP (Critical for multi-model run)
        del llm
        gc.collect()
        print("🧹 VRAM Flushed.")
